...
-- truybn bo sung
If not exists (SELECT 1 FROM sys.columns WHERE name = 'ecsBIEU_THUE_XNK' and object_id = (select object_id from sys.objects where object_id= OBJECT_ID(N'[dbo].[DHANGMDDC]') AND type in (N'U')))
Begin
Alter Table [DHANGMDDC] add ecsBIEU_THUE_XNK varchar(50) NULL
Insert Into @tblColumnAdded
Values ('--Table [DHANGMDDC] da duoc bo sung them Column [ecsBIEU_THUE_XNK]')
End

If not exists (SELECT 1 FROM sys.columns WHERE name = 'ecsBIEU_THUE_TTDB' and object_id = (select object_id from sys.objects where object_id= OBJECT_ID(N'[dbo].[DHANGMDDC]') AND type in (N'U')))
Begin
Alter Table [DHANGMDDC] add ecsBIEU_THUE_TTDB varchar(50) NULL
Insert Into @tblColumnAdded
Values ('--Table [DHANGMDDC] da duoc bo sung them Column [ecsBIEU_THUE_TTDB]')
End

If not exists (SELECT 1 FROM sys.columns WHERE name = 'ecsBIEU_THUE_VAT' and object_id = (select object_id from sys.objects where object_id= OBJECT_ID(N'[dbo].[DHANGMDDC]') AND type in (N'U')))
Begin
Alter Table [DHANGMDDC] add ecsBIEU_THUE_VAT varchar(50) NULL
Insert Into @tblColumnAdded
Values ('--Table [DHANGMDDC] da duoc bo sung them Column [ecsBIEU_THUE_VAT]')
End
-- end truybn
