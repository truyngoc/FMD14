...
exec('Select t.* into DToKhaiMD From ' + @DBNameSXXK + 'DToKhaiMD t join ' + @DBNameSXXK + 'CDDL_DSToKhai ds on t.SoTK=ds.SoTK and t.NamDK=ds.NamDK and t.Ma_HQ=ds.Ma_HQ and t.Ma_LH=ds.Ma_LH')-- Where Thanh_Ly<>''H''')-- and Ma_HQ in (' + @MaHQs + ')')
...
exec('Select e.* into ecsToKhaiMD From ' + @DBNameSLXNK + 'ecsToKhaiMD e join ' + @DBNameSXXK + 'CDDL_DSToKhai ds on e.SoTK=ds.SoTK and e.NamDK=ds.NamDK and e.Ma_HQ=ds.Ma_HQ and e.Ma_LH=ds.Ma_LH')-- join DToKhaiMD t on e.SoTK=t.SoTK and e.NamDK=t.NamDK and e.Ma_HQ=t.Ma_HQ and e.Ma_LH=t.Ma_LH')-- and e.Ma_HQ in (' + @MaHQs + ')')
...
print convert(varchar(30),getdate(),114) + N' - 3. Start DHANGMDDK'
--hàng DK
exec('select h.* into DHANGMDDK from '+ @DBNameSXXK +'DHANGMDDK h join ' + @DBNameSXXK + 'DToKhaiMD t on h.SoTK = t.SoTK and h.Ma_LH = t.Ma_LH and h.Ma_HQ = t.Ma_HQ and h.NamDK = t.NamDK')-- and h.Ma_HQ in (' + @MaHQs + ')' )
...
--hàng TH
exec('select h.* into DHangMDTH from '+ @DBNameSXXK +'DHangMDTH h join ' + @DBNameSXXK + 'CDDL_DSToKhai ds on h.SoTK=ds.SoTK and h.NamDK=ds.NamDK and h.Ma_HQ=ds.Ma_HQ and h.Ma_LH=ds.Ma_LH Where h.Ma_LH like ''NKD%'' or h.Ma_LH like ''X%''')--' + @DBNameSXXK + 'DToKhaiMD t on h.SoTK = t.SoTK and h.Ma_LH = t.Ma_LH and h.Ma_HQ = t.Ma_HQ and h.NamDK = t.NamDK Where t.Thanh_Ly<>''H'' and (h.Ma_LH like ''NKD%'' or h.Ma_LH like ''X%'')')-- and h.Ma_HQ in (' + @MaHQs + ')' )
...
FROM cte h join ' + @DBNameSXXK + 'CDDL_DSToKhai ds on h.SoTK=ds.SoTK and h.NamDK=ds.NamDK and h.Ma_HQ=ds.Ma_HQ and h.Ma_LH=ds.Ma_LH and h.rn = 1
Where h.Ma_LH like ''NSX%''')
...
--NPL tồn
exec('select h.* into DNPLNHAP from '+ @DBNameSXXK +'DNPLNHAP h join ' + @DBNameSXXK + 'CDDL_DSToKhai ds on h.SoTK=ds.SoTK and h.NamDK=ds.NamDK and h.Ma_HQ=ds.Ma_HQ and h.Ma_LH=ds.Ma_LH Where h.TON>0')-- and Ma_HQ in (' + @MaHQs + ')')
