...
-- to khai che xuat
exec('Select t.* into DToKhaiMD From ' + @DBName + 'DToKhaiMD t join '+ @DBName +'CDDL_DSHDGC h on t.So_HD=h.So_HD and t.Ma_HQ=h.Ma_HQHD and t.Ma_DV=h.DVGC and t.Ngay_HD=h.Ngay_Ky Where Ma_LH like ''_GC%'' and t.Ma_HQ in (' + @Ma_HQs + ')')
...
exec('select top 0 * into ecsCX_HUY_NPLSP_D from '+ @DBName +'ecsCX_HUY_NPLSP_D')		
