...
REM set sxxk_dbname_src="DanhMucSXXK"
...
:: SLXNK
sqlcmd -U %usr_src% -P %pwd_src% -S %lnksrv_datasource% -d %slxnk_dbname_src% -i "TC.03.SLXNK_CC Kiem tra MaHQ.sql" -b >> "%IDCucHQ%_%IDChiCuc%_ServerProcess_log.txt"
:: SXXK
...
:: thao tac tren server 01
sqlcmd -U %usr01% -P %pwd01% -S %srv01% -d %dbs01%  -i "TC.05.eCustoms Server 1 import to khai GC CX.sql" -v lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src% -b >> "%IDCucHQ%_%IDChiCuc%_Server01_log.txt"

::==========================================================

echo --- 2.Execution on Server 02 ---
echo --- 1.1.Check LinkServer on Server 02 --- >> "%IDCucHQ%_%IDChiCuc%_Server02_log.txt"
sqlcmd -U %usr02% -P %pwd02% -S %srv02% -d %dbs02%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server02_log.txt"

echo --- 1.2.Sync GCCX category data on Server 02 --- >> "%IDCucHQ%_%IDChiCuc%_Server02_log.txt"
sqlcmd -U %usr02% -P %pwd02% -S %srv02% -d %dbs02%  -i "TC.05.eCustoms Server 2 import to khai GC CX.sql" -v lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server02_log.txt"
::==========================================================

echo --- 3.Execution on Server 03 ---
echo --- 1.1.Check LinkServer on Server 03 --- >> "%IDCucHQ%_%IDChiCuc%_Server03_log.txt"
sqlcmd -U %usr03% -P %pwd03% -S %srv03% -d %dbs03%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server03_log.txt"

echo --- 1.2.Sync GCCX category data on Server 03 --- >> "%IDCucHQ%_%IDChiCuc%_Server03_log.txt"
sqlcmd -U %usr03% -P %pwd03% -S %srv03% -d %dbs03%  -i "TC.05.eCustoms Server 3 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server03_log.txt"
::==========================================================

echo --- 4.Execution on Server 04 ---
echo --- 1.1.Check LinkServer on Server 04 --- >> "%IDCucHQ%_%IDChiCuc%_Server04_log.txt"
sqlcmd -U %usr04% -P %pwd04% -S %srv04% -d %dbs04%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server04_log.txt"

echo --- 1.2.Sync GCCX category data on Server 04 --- >> "%IDCucHQ%_%IDChiCuc%_Server04_log.txt"
sqlcmd -U %usr04% -P %pwd04% -S %srv04% -d %dbs04%  -i "TC.05.eCustoms Server 4 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server04_log.txt"
::==========================================================

echo --- 5.Execution on Server 05 ---
echo --- 1.1.Check LinkServer on Server 05 --- >> "%IDCucHQ%_%IDChiCuc%_Server05_log.txt"
sqlcmd -U %usr05% -P %pwd05% -S %srv05% -d %dbs05%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server05_log.txt"

echo --- 1.2.Sync GCCX category data on Server 05 --- >> "%IDCucHQ%_%IDChiCuc%_Server05_log.txt"
sqlcmd -U %usr05% -P %pwd05% -S %srv05% -d %dbs05%  -i "TC.05.eCustoms Server 5 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server05_log.txt"
::==========================================================

echo --- 6.Execution on Server 06 ---
echo --- 1.1.Check LinkServer on Server 06 --- >> "%IDCucHQ%_%IDChiCuc%_Server06_log.txt"
sqlcmd -U %usr06% -P %pwd06% -S %srv06% -d %dbs06%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server06_log.txt"

echo --- 1.2.Sync GCCX category data on Server 06 --- >> "%IDCucHQ%_%IDChiCuc%_Server06_log.txt"
sqlcmd -U %usr06% -P %pwd06% -S %srv06% -d %dbs06%  -i "TC.05.eCustoms Server 6 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server06_log.txt"
::==========================================================

echo --- 7.Execution on Server 07 ---
echo --- 1.1.Check LinkServer on Server 07 --- >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"
sqlcmd -U %usr07% -P %pwd07% -S %srv07% -d %dbs07%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"

echo --- 1.2.Sync GCCX category data on Server 07 --- >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"
sqlcmd -U %usr07% -P %pwd07% -S %srv07% -d %dbs07%  -i "TC.05.eCustoms Server 7 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"
::==========================================================

echo --- 8.Execution on Server 08 ---
echo --- 1.1.Check LinkServer on Server 08 --- >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
sqlcmd -U %usr08% -P %pwd08% -S %srv08% -d %dbs08%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"

echo --- 1.2.Sync GCCX category data on Server 08 --- >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
sqlcmd -U %usr08% -P %pwd08% -S %srv08% -d %dbs08%  -i "TC.05.eCustoms Server 8 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
::==========================================================

echo --- 9.Execution on Server 09 ---
echo --- 1.1.Check LinkServer on Server 09 --- >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
sqlcmd -U %usr09% -P %pwd09% -S %srv09% -d %dbs09%  -i "SLXNK.Linkserver ECustoms_Online to SLXNK.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"

echo --- 1.2.Sync GCCX category data on Server 09 --- >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
sqlcmd -U %usr09% -P %pwd09% -S %srv09% -d %dbs09%  -i "TC.05.eCustoms Server 9 import to khai GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
::==========================================================
