...
REM sqlcmd -U %usr% -P %pwd% -S %srv% -d %dbs%  -i "C.03.SLXNK_CC. Report so lieu.sql" -b >> "%IDCucHQ%_%IDChiCuc%_Report_log.txt"
REM ::==========================================================
...
sqlcmd -U %usr% -P %pwd% -S %srv% -d %dbs%  -i "TC.02.SLXNK_CC Gen column to khai.sql" -v IDCucHQ=%IDCucHQ% IDChiCuc=%IDChiCuc% -b >> "%IDCucHQ%_%IDChiCuc%_Gen_column_log.txt"
::==========================================================
sqlcmd -U %usr% -P %pwd% -S %srv% -d %dbs% -i "TC.03.SLXNK_CC Kiem tra MaHQ.sql" -b >> "%IDCucHQ%_%IDChiCuc%_ServerProcess_log.txt"
