...
( NAME = N'TOKHAIGCCX', FILENAME = N'D:\ToKhaiDiaPhuong\Dong Nai\C47D02\TOKHAIGCCX.mdf' , SIZE = 2048KB , FILEGROWTH = 1024KB )
 LOG ON 
( NAME = N'TOKHAIGCCX_log', FILENAME = N'D:\ToKhaiDiaPhuong\Dong Nai\C47D02\TOKHAIGCCX_log.ldf' , SIZE = 1024KB , FILEGROWTH = 10%)
GO
...
-- to khai che xuat
exec('Select t.* into DToKhaiMD From ' + @DBName + 'DToKhaiMD t join '+ @DBName +'DHDGC h on t.So_HD=h.So_HD and t.Ma_HQ=h.Ma_HQHD and t.Ma_DV=h.DVGC and t.Ngay_HD=h.Ngay_Ky Where Ma_LH like ''_GC%'' and h.ThanhKhoan<>1' + ' and t.Ma_HQ in (' + @Ma_HQs + ')')

exec('insert into DToKhaiMD select * from ' + @DBName + 'DToKhaiMD where Ma_LH like ''_CX%''' + ' and Ma_HQ in (' + @Ma_HQs + ')')
