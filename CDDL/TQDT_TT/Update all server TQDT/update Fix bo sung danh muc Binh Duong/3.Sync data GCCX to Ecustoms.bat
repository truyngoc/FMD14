...
REM sqlcmd -U %usr07% -P %pwd07% -S %srv07% -d %dbs07%  -i "SLXNK.Linkserver eCustoms to DanhMucGCCX.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"

REM echo --- 1.2.Sync GCCX category data on Server 07 --- >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"
REM sqlcmd -U %usr07% -P %pwd07% -S %srv07% -d %dbs07%  -i "TC.03.eCustoms Server 7 import du lieu lan DAU GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server07_log.txt"
REM ::==========================================================

REM echo --- 8.Execution on Server 08 ---
REM echo --- 1.1.Check LinkServer on Server 08 --- >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
REM sqlcmd -U %usr08% -P %pwd08% -S %srv08% -d %dbs08%  -i "SLXNK.Linkserver eCustoms to DanhMucGCCX.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"

REM echo --- 1.2.Sync GCCX category data on Server 08 --- >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
REM sqlcmd -U %usr08% -P %pwd08% -S %srv08% -d %dbs08%  -i "TC.03.eCustoms Server 8 import du lieu lan DAU GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server08_log.txt"
REM ::==========================================================

REM echo --- 9.Execution on Server 09 ---
REM echo --- 1.1.Check LinkServer on Server 09 --- >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
REM sqlcmd -U %usr09% -P %pwd09% -S %srv09% -d %dbs09%  -i "SLXNK.Linkserver eCustoms to DanhMucGCCX.sql" -v lnksrv_name=%lnksrv_name% datasource=%lnksrv_datasource% uid=%usr_src% pwd=%pwd_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"

REM echo --- 1.2.Sync GCCX category data on Server 09 --- >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
REM sqlcmd -U %usr09% -P %pwd09% -S %srv09% -d %dbs09%  -i "TC.03.eCustoms Server 9 import du lieu lan DAU GC CX.sql" -v  lnksrv_name=%lnksrv_name% slxnk_dbname_src=%slxnk_dbname_src%  -b >> "%IDCucHQ%_%IDChiCuc%_Server09_log.txt"
REM ::==========================================================
