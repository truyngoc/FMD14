...
( NAME = N'DanhMucSXXK', FILENAME = N'D:\SQLDatas\DanhMucSXXK.mdf' , SIZE = 4096KB , FILEGROWTH = 1024KB )
 LOG ON 
( NAME = N'DanhMucSXXK_log', FILENAME = N'D:\SQLDatas\DanhMucSXXK_log.ldf' , SIZE = 2048KB , FILEGROWTH = 10%)
GO
...
exec('select * into SNPL from ' + @DBName + 'SNPL Where Ma_HQ in (' + @MaHQs + ')')

exec('select * into  SSP from ' + @DBName + 'SSP Where Ma_HQ in (' + @MaHQs + ')')

exec('select * into DDINHMUC from ' + @DBName + 'DDINHMUC Where Ma_HQ in (' + @MaHQs + ')')
