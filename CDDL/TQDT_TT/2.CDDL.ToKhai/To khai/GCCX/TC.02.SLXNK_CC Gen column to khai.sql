...
Update DToKhaiMD
	 Set	CDDL_TK_ID		= @IDDanhMuc + CDDL_ID
			...
			,CDDL_MA_HQ		= dbo.Doi_Ma_HQ(rtrim(SUBSTRING(ma_hq,2,2) + SUBSTRING(ma_hq,1,1) + SUBSTRING(ma_hq,4,10)))
			...
			,CDDL_SO_HD	= dbo.fcnTCVN2Unicode(LEFT(SO_HD,50))
...
From DToKhaiMD t join DGC_HD h on t.CDDL_MA_HQ = rtrim(h.Ma_HQ_HD) and t.Ma_DV=h.Don_Vi_GC and dbo.fcnTCVN2Unicode(t.So_HD)=h.So_HD and t.Ngay_HD=h.Ngay_Ky
...
Set		h.CDDL_TK_ID	= t.CDDL_TK_ID
		,h.CDDL_MA_HQ	= dbo.Doi_Ma_HQ(rtrim(SUBSTRING(h.ma_hq,2,2) + SUBSTRING(h.ma_hq,1,1) + SUBSTRING(h.ma_hq,4,10)))
...
From	DHangMD h join DToKhaiMD t on t.SoTK = h.SoTK and t.Ma_LH=h.Ma_LH and rtrim(t.Ma_HQ)=rtrim(h.Ma_HQ) and t.NamDK=h.NamDK
...
Update h
Set		h.CDDL_VT_ID = n.VT_ID
From	DHangMD h join DCX_DM_HH_DUA_VAO_RA n on h.CDDL_MA_HQ=rtrim(n.MA_HQ) and h.CDDL_Ma_Don_Vi=n.MA_DV and h.CDDL_Ma_Vat_Tu=n.MA_HANG
where	h.Ma_LH like '_CX%'

-- gia cong
update n
set 
		n.CDDL_MA_HQ	= dbo.Doi_Ma_HQ(rtrim(SUBSTRING(n.ma_hq,2,2) + SUBSTRING(n.ma_hq,1,1) + SUBSTRING(n.ma_hq,4,10)))
...
set 
		...
		,n.CDDL_HD_ID = t.CDDL_HD_ID
from ecsNPLTCU n join DToKhaiMD t on t.SoTK = n.SoTK and t.Ma_LH=n.Ma_LH and rtrim(t.Ma_HQ)=rtrim(n.Ma_HQ) and t.NamDK=n.NamDK


-- update n
-- set 
		-- n.CDDL_VT_ID = d.VT_ID
-- from ecsNPLTCU n join DGC_SP_NPL_TB_HM d on n.CDDL_MA_HQ=rtrim(d.Ma_HQ_HD) and n.CDDL_Ma_Vat_Tu=d.Ma_Vat_Tu and d.Loai=1
update h
set 
		h.CDDL_VT_ID = n.VT_ID
from ecsNPLTCU h join DGC_SP_NPL_TB_HM n on h.CDDL_HD_ID=n.HD_ID and h.CDDL_Ma_Vat_Tu=n.Ma_Vat_Tu and n.Loai = 1
...
from ecsNPLTCU_CT t join ecsNPLTCU n on n.SoTK = t.SoTK and n.Ma_LH = t.Ma_LH and rtrim(n.Ma_HQ) = rtrim(t.Ma_HQ) and n.NamDK = t.NamDK and n.NPLP_Code = t.NPLP_Code

-- che xuat
Update ecsCX_HUY_NPLSP_D
Set	
	CDDL_CT_ID			= @IDDanhMuc + 900000000 + CDDL_ID
	,CDDL_MA_HQ			= dbo.Doi_Ma_HQ(rtrim(SUBSTRING(MA_HQTN,2,2) + SUBSTRING(MA_HQTN,1,1) + SUBSTRING(MA_HQTN,4,10)))
...
from ecsCX_HUY_NPLSP_D h
	join DToKhaiMD d on rtrim(h.MA_HQTN)=rtrim(d.Ma_HQ)
				and h.MA_LH=d.Ma_LH 
...
from ecsCX_HUY_NPLSP_D h
	join DCX_DM_HH_DUA_VAO_RA v on h.MA_HANG = v.MA_HANG 
							...
							and h.CDDL_MA_HQ = rtrim(v.MA_HQ)
