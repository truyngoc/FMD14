...
from vDSX_NPL_NHAP_TL	where TK_ID>@IDDanhMuc and TK_ID<=@IDDanhMucMax
...
from ECUSTOMS_HIS.dbo.DSX_NPL_NHAP_TL	where TK_ID>@IDDanhMuc and TK_ID<=@IDDanhMucMax
...
declare cur cursor for
	...
	from 
		...
		group by ma_hq, ma_lh	
		union
		select ma_hq, ma_lh, 0 as sl_tk, 0 as sl_h, COUNT(*) as sl_nplton from vDSX_NPL_NHAP_TL
		where TK_ID>@IDDanhMuc and TK_ID<=@IDDanhMucMax
		group by ma_hq, ma_lh
		union
		...
		group by ma_hq, ma_lh	
		union
		select ma_hq, ma_lh, 0 as sl_tk, 0 as sl_h, COUNT(*) as sl_nplton from ECUSTOMS_HIS.dbo.DSX_NPL_NHAP_TL
		where TK_ID>@IDDanhMuc and TK_ID<=@IDDanhMucMax
		group by ma_hq, ma_lh
		) a
