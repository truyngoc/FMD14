...
if exists(select * from sys.columns 
             where Name = N'CDDL_So_HD' and Object_ID = Object_ID(N'DHDGC'))
begin
	alter table dbo.DHDGC drop column CDDL_So_HD
	
	ALTER TABLE dbo.DHDGC ADD
				CDDL_So_HD nvarchar(80)
end	
else
begin
	ALTER TABLE dbo.DHDGC ADD
...
Update DHDGC
	 Set	CDDL_HD_ID		= @IDDanhMuc + CDDL_ID
			...
			,CDDL_MA_HQ_HD	= rtrim(SUBSTRING(ma_hqhd,2,2) + SUBSTRING(ma_hqhd,1,1) + SUBSTRING(ma_hqhd,4,10))
			,CDDL_So_HD		= dbo.fcnTCVN2Unicode(So_HD)
			,CDDL_So_HD_HQ_Cap = CONVERT(VARCHAR(32),HashBytes('MD5', (rtrim(SUBSTRING(ma_hqhd,2,2) + SUBSTRING(ma_hqhd,1,1) + SUBSTRING(ma_hqhd,4,10)) +'|'+DVGC+'|'+ dbo.fcnTCVN2Unicode(So_HD)+'|'+CAST(datepart(year, Ngay_Ky) as varchar(4))+RIGHT('00'+CAST(datepart(M, Ngay_Ky) as varchar(2)),2)+RIGHT('00'+ CAST(datepart(d, Ngay_Ky) as varchar(2)),2))),2)
...
From DLOAISPGC a join DHDGC hd on a.So_HD=hd.So_HD and rtrim(a.Ma_HQHD)=rtrim(hd.Ma_HQHD) and a.DVGC=hd.DVGC and a.Ngay_Ky=hd.Ngay_Ky
...
From DNPLHD a join DHDGC hd on a.So_HD=hd.So_HD and rtrim(a.Ma_HQHD)=rtrim(hd.Ma_HQHD) and a.DVGC=hd.DVGC and a.Ngay_Ky=hd.Ngay_Ky
...
set	a.CDDL_HD_ID		= hd.CDDL_HD_ID
	...
	,a.CDDL_MA_HQ_HD	= hd.CDDL_MA_HQ_HD
	,a.CDDL_P_CODE		= SUBSTRING(a.P_CODE,2,50)
From DSPGC a join DHDGC hd on a.So_HD=hd.So_HD and rtrim(a.Ma_HQHD)=rtrim(hd.Ma_HQHD) and a.DVGC=hd.DVGC and a.Ngay_Ky=hd.Ngay_Ky
...
From DThietBi a join DHDGC hd on a.So_HD=hd.So_HD and rtrim(a.Ma_HQHD)=rtrim(hd.Ma_HQHD) and a.DVGC=hd.DVGC and a.Ngay_Ky=hd.Ngay_Ky
...
set	a.CDDL_HD_ID		= hd.CDDL_HD_ID
	...
	,a.CDDL_MA_HQ_HD	= hd.CDDL_MA_HQ_HD
	,a.CDDL_P_CODE		= SUBSTRING(a.P_CODE,2,50)
From ecsDHangMau a join DHDGC hd on a.So_HD=hd.So_HD and rtrim(a.Ma_HQHD)=rtrim(hd.Ma_HQHD) and a.DVGC=hd.DVGC and a.Ngay_Ky=hd.Ngay_Ky
...
from DDMuc dm join DHDGC hd on dm.So_HD=hd.So_HD and rtrim(dm.Ma_HQHD)=rtrim(hd.Ma_HQHD) and dm.DVGC=hd.DVGC and dm.Ngay_Ky=hd.Ngay_Ky
...
set
	...
	,CDDL_MA_HQ	= rtrim(SUBSTRING(MA_HQTN,2,2) + SUBSTRING(MA_HQTN,1,1) + SUBSTRING(MA_HQTN,4,10))
...
from ecsCX_DinhMuc d
	join ecsCX_DM_HH_DUA_VAO_D v on d.MA_NPL = v.MA_HANG and d.MA_DV = v.MA_DV_XN and rtrim(d.MA_HQ) = rtrim(v.MA_HQTN) and LOAI_HANG = 1 -- NPL
...
from ecsCX_DinhMuc d
	join ecsCX_DM_HH_DUA_RA_D r on d.MA_SP = r.MA_HANG and d.MA_DV = r.MA_DV_XN and rtrim(d.MA_HQ) = rtrim(r.MA_HQTN) and LOAI_HANG = 2 -- SP
...
from ecsCX_CT_TOKHAI_HANG h
	join ecsCX_CT_TOKHAI t on h.SO_TN = t.SO_TN and h.NAM_TN = t.NAM_TN and rtrim(h.MA_HQTN) = rtrim(t.MA_HQTN)
...
from ecsCX_CT_TOKHAI_HANG h
	...
	join ecsCX_DM_HH_DUA_VAO_D v on h.MA_HANG = v.MA_HANG and t.MA_DV_XN = v.MA_DV_XN and rtrim(h.MA_HQTN) = rtrim(v.MA_HQTN) and h.LOAI_HANG = v.LOAI_HANG
...
from ecsCX_CT_TOKHAI_HANG h
	join ecsCX_CT_TOKHAI t on h.SO_TN = t.SO_TN and h.NAM_TN = t.NAM_TN and h.MA_HQTN = t.MA_HQTN	-- join de lay MA_DV_XN
	join ecsCX_DM_HH_DUA_RA_D v on h.MA_HANG = v.MA_HANG and t.MA_DV_XN = v.MA_DV_XN and rtrim(h.MA_HQTN) = rtrim(v.MA_HQTN) and h.LOAI_HANG = v.LOAI_HANG
...
BEgin
			...
			update DHDGC 
			set CDDL_MA_HQ_HD = dbo.Doi_Ma_HQ(rtrim(CDDL_MA_HQ_HD))
				,CDDL_So_HD_HQ_Cap = CONVERT(VARCHAR(32),HashBytes('MD5', (dbo.Doi_Ma_HQ(rtrim(CDDL_MA_HQ_HD)) +'|'+DVGC+'|'+ dbo.fcnTCVN2Unicode(So_HD)+'|'+CAST(datepart(year, Ngay_Ky) as varchar(4))+RIGHT('00'+CAST(datepart(M, Ngay_Ky) as varchar(2)),2)+RIGHT('00'+ CAST(datepart(d, Ngay_Ky) as varchar(2)),2))),2)
